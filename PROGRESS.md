...
| 2026-09-05 | **P9+P10+P11 全量部署上线**（版本 11b4dcfe）并远程应用 0005/0006/0007 迁移（deploy 与迁移背靠背执行，group_name 读窗口秒级）：远程 schema 校验通过（2 个 identity 索引、boards.group_name 存在且 9 版块全有分组、identities 无 post_count）；线上只读回归 11 项全绿（15 路径 200+404、Set-Cookie Secure + CSP frame-ancestors + X-Robots-Tag、左栏 3 分组 9 版块 DB 驱动、热帖榜「N 抱抱」、面包屑趣味区、**楼层时间北京时间核验 UTC 09:36→渲染 17:36**、/new 默认与 ?board= 预选、搜索「共 5 条」、/me 身份码卡片与重置确认链接、未登录 /mod/delete-confirm 回跳 /mod、robots 200）。全程未在线上做任何写操作（无发帖/抱抱/站务登录）。待用户推送 GitHub 触发 CI 首跑 |
| 2026-09-05 | **P11 收官**（7 切片均过门，待部署）：P11-1 时间按 Asia/Shanghai 渲染 + 上海日界 + joinDays UTC 解析；P11-2 safeReturn 反斜杠归一 / 引用快照服务端唯一生成 / reason 截断 / toggleHug 存在性校验 / Secure cookie / frame-ancestors / 回复失败提示条（P11-4 项前移）；P11-3 最后回复窗口函数化 + 0005 identity 索引 + flushViews 分批限子请求预算（20 键/次）+ 周抱抱口径补楼层；P11-4 版块上下文预选 / chips :has() 选中态 / 失败保留输入；P11-5 热帖榜抱抱标签（HotItem.replies→hugs）+ 搜索真实 total + 面包屑分组推导 + 我回应的 #floor 锚点；P11-6 版块分组入库（0006 boards.group_name，navGroups/NavGroup/boardGroupName 退役）+ post_count 死列清理（0007，Identity.totalPosts 改可选）+ getBoardHot 收敛 + 死 CSS + 缓存键 v2；P11-7 observability + CI concurrency + 重置/删除两步确认页 + 分页窗口化（lib/pagination + 共享组件，测试 80 用例）。**用户侧上线顺序：① deploy → ② d1 migrations apply --remote（0005/0006/0007）→ ③ 只读回归**（清单见 P11-7 注记）；0007 必须在部署后应用 |
