...
- [-] **P15-2 回复框重设计**：① 根因修复 .reply-box width:100%（缺宽致 textarea 回落默认 ~20 字符宽，截图诊断）+ rows 4 + resize:vertical + 14px 主文字色（原 13px 浅灰误用占位色）；② 聚焦态纯 CSS（:focus 白底+主色边框）覆盖回复框/发帖正文/标题/验证码/登录/口令/举报理由——无 JS 架构下的「系统在响应」即时反馈；③ 发帖正文 180→220px；④ 回复 note 补「最多 300 字」；⑤ DESIGN.md §2 输入控件规范。门：tsc + npm test + 冒烟（样式断言）
