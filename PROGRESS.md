...
- [-] **P13-3 盖楼分页**：① getThreadDetail 楼层查询改 `?page=` 每页 20 楼（floor 区间），meta 增加总页数；② 详情页复用 P11-7 共享分页组件（链接保留 op=1）；③ 通知跳转 /notifications/open 按楼层号换算目标页；④ 引用预览/只看楼主不受影响（op 视图不翻页）；⑤ 自伤横幅语义调整为「楼主+当前页可见楼层」并记入 ARCHITECTURE §5。门：tsc + npm test + 冒烟（>20 楼帖分页渲染、跨页楼层锚点换算、op 视图）
