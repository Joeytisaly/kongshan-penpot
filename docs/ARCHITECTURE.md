...
| 回复 IP 限流 | IP-HMAC 30 条回复/小时（回复原仅按身份限频，懒签发换身份即绕过——P12-2 补口） | ✓ P12-2 |
...
以下为十二角色审查确认、经权衡后**有意暂不实现**的能力，记录在此防止后续重复「发现」：
...
- **站务会话无服务端吊销**：mod_auth 为无状态签名令牌，「退出」= 浏览器删 cookie；已拷贝
  的令牌值在 24h 过期前仍可重放（与 JWT 同理）。触发条件：出现令牌泄露事故时加 KV 黑名单
  或令牌版本号。
